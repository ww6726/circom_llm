...
void powOfTwo_6_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void powOfTwo_6_run(uint ctx_index,Circom_CalcWit* ctx);
void rightShift_7_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void rightShift_7_run(uint ctx_index,Circom_CalcWit* ctx);
void shift_test_8_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void shift_test_8_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 347;}

uint get_number_of_components() {return 73;}
...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 65],&lvar[3]); // line circom 34
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 65]); // line circom 39
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[5]); // line circom 62
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[1];
// load src
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[5]); // line circom 262
while(Fr_isTrue(&expaux[0])){
{
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[2]); // line circom 262
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[5]); // line circom 262
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_sub(&expaux[4],&circuitConstants[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[33]].signalStart + 0]); // line circom 277
Fr_mul(&expaux[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[34]].signalStart + 0],&expaux[4]); // line circom 277
Fr_mul(&expaux[1],&expaux[2],&circuitConstants[6]); // line circom 277
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 34]); // line circom 277
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 35; i++){
...
powOfTwo_6_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 266 ;
aux_cmp_num += 68;
}
...
// need to run sub component
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1));
powOfTwo_6_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_idiv(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 3]); // line circom 293
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mod(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 3]); // line circom 294
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_mul(&expaux[3],&signalValues[mySignalStart + 0],&signalValues[mySignalStart + 3]); // line circom 295
Fr_add(&expaux[2],&expaux[3],&signalValues[mySignalStart + 4]); // line circom 295
Fr_eq(&expaux[0],&signalValues[mySignalStart + 1],&expaux[2]); // line circom 295
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template/function " << myTemplateName << " line 295. " <<  "Followed trace of components: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
for (uint i = 0; i < 2; i++){
uint index_subc = ctx->componentMemory[ctx_index].subcomponents[i];
if (index_subc != 0)release_memory_component(ctx,index_subc);
...
ctx->componentMemory[coffset].templateName = "shift_test";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
rightShift_7_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 343 ;
aux_cmp_num += 72;
}
...
// need to run sub component
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1));
rightShift_7_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
...
shift_test_8_create(1,0,ctx,"main",0);
shift_test_8_run(0,ctx);
}
