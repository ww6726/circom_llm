...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 9],&lvar[3]); // line circom 34
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 9]); // line circom 39
...
ctx->componentMemory[coffset].templateName = "fixPointDiv";
ctx->componentMemory[coffset].signalStart = soffset;
...
// need to run sub component
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1));
fixPointDiv_2_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
