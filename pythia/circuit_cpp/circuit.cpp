...
void absoluteValue_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void absoluteValue_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
void truncate_5_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void truncate_5_run(uint ctx_index,Circom_CalcWit* ctx);
void abs_div_test_6_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void abs_div_test_6_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 119;}
...
// load src
Fr_mul(&expaux[1],&circuitConstants[5],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0]); // line circom 48
Fr_sub(&expaux[0],&expaux[1],&circuitConstants[2]); // line circom 48
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[6]); // line circom 33
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 0)];
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 32],&lvar[3]); // line circom 34
Fr_band(&expaux[0],&expaux[1],&circuitConstants[2]); // line circom 34
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 32]); // line circom 39
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template/function " << myTemplateName << " line 39. " <<  "Followed trace of components: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
for (uint i = 0; i < 0; i++){
uint index_subc = ctx->componentMemory[ctx_index].subcomponents[i];
...
absoluteValue_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 76 ;
aux_cmp_num += 4;
}
...
// need to run sub component
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1));
absoluteValue_3_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
// load src
Fr_idiv(&expaux[0],&signalValues[mySignalStart + 2],&circuitConstants[9]); // line circom 72
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mod(&expaux[0],&signalValues[mySignalStart + 2],&circuitConstants[9]); // line circom 73
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_mul(&expaux[3],&signalValues[mySignalStart + 4],&circuitConstants[9]); // line circom 74
Fr_add(&expaux[2],&expaux[3],&signalValues[mySignalStart + 5]); // line circom 74
Fr_eq(&expaux[0],&signalValues[mySignalStart + 2],&expaux[2]); // line circom 74
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 4],&signalValues[mySignalStart + 3]); // line circom 77
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 2; i++){
...
ctx->componentMemory[coffset].templateName = "abs_div_test";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint csoffset = mySignalStart+3;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "trun";
truncate_5_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 115 ;
aux_cmp_num += 6;
}
...
Fr_idiv(&expaux[0],&signalValues[mySignalStart + 2],&circuitConstants[9]); // line circom 161
{
...
// need to run sub component
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1));
truncate_5_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
abs_div_test_6_create(1,0,ctx,"main",0);
abs_div_test_6_run(0,ctx);
}
