...
void shift_test_1_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void shift_test_1_run(uint ctx_index,Circom_CalcWit* ctx);
...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 16],&lvar[3]); // line circom 34
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 16]); // line circom 39
...
// load src
Fr_shl(&expaux[0],&circuitConstants[3],&signalValues[mySignalStart + 2]); // line circom 135
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[0]); // line circom 141
while(Fr_isTrue(&expaux[0])){
{
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[2]); // line circom 145
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
shift_test_1_create(1,0,ctx,"main",0);
shift_test_1_run(0,ctx);
}
